...
class SaleOrder(models.Model):
    ...
    approval_user_line_ids = fields.One2many('approvehub.sale.user.line', 'sale_order_id', string='Users Line')
    ...
    def _compute_is_visible_buttons(self):
        for order in self:
            approvehub_form = self.env['approvehub.form'].sudo().search([('model_id.model', '=', 'sale.order')],
                                                                        limit=1)
    ...
    def default_get(self, fields_list):
        ...
        approval_id = self.env['approvehub.form'].sudo().search([('model_id.model', '=', 'sale.order')], limit=1)
        approval_line_list = []
        if approval_id:
            if approval_id.user_ids:
                for user_line in approval_id.user_ids:
                    if user_line.status == 'mandatory':
                        ...
                        approval_user_line_id = self.env['approvehub.sale.user.line'].sudo().create(user_line_values)
    ...
    def action_approve(self):
        ...
        for order in self:
            ...
            print('mandatory_user_lines-------------------------------------',mandatory_user_lines)
            if logged_in_user in mandatory_user_lines.mapped('user_id'):
                ...
                approval_form = self.env['approvehub.form'].sudo().search([('model_id.model', '=', 'sale.order')], limit=1)
