...
class RejectionReasonWizard(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        if self.env.context.get('active_model') == 'purchase.order' and self.env.context.get('active_id'):
            result['purchase_order_id'] = self.env['purchase.order'].sudo().browse(self.env.context.get('active_id')).id
    ...
    def action_submit_rejection_reason(self):
        ...
        if self.purchase_order_id:
            user_line = self.env['approvehub.purchase.user.line'].sudo().search([
        ...
        if user_line:
            ...
            if self.purchase_order_id:
                self.purchase_order_id.state = 'rejected'
            user_line.rejection_reason = self.reason

            context = {'reason': self.reason}
            if self.purchase_order_id:
                template = self.env.ref('pragtech_approval_hub_purchase.reject_purchase_order_email_template')
                self.purchase_order_id.write({'reason': self.reason})

            template.with_context(context).send_mail(self.purchase_order_id.id,force_send=True)
